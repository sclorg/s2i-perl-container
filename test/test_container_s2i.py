...
bin_app = TEST_DIR / "binpath"
fcgi = TEST_DIR / "fcgi"
psgi = TEST_DIR / "psgi"
psgi_hot_deploy = TEST_DIR / "psgi-hot_deploy"
psgi_variables = TEST_DIR / "psgi-variables"
warningonstderr = TEST_DIR / "warningonstderr"
...
def build_npm_app(app_path: Path) -> ContainerTestLib:
    ...
    s2i_app = container_lib.build_as_df(
        ...
        dst_image=f"{IMAGE_NAME}-testapp"
    )
    return s2i_app
...
class TestPerlNPMtestContainer:
    ...
    def test_npm_works(self):
        assert self.s2i_app.npm_works(image_name=IMAGE_NAME)
